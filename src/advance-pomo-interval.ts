...
function advancePomoInterval() {
    if (data.currentPomoInterval === 'workTime') { // just finished a work cycle
        ...
        if (data.pomoIntervalCounter !== 4) { // need a short break -- 4 cycles not completed yet
            const result = prompt(`Work-time #${data.pomoIntervalCounter} is over. Well done! Press enter to start your short break or type "quit" to exit out. `);
            if (result === '') {
        ...
        const result = prompt(`Short-break #${data.pomoIntervalCounter} is over -- time to go back to work. Press enter to start work-time or type "quit" to exit out. `);
        if (result === '') {
